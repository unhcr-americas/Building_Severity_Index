...
subtitle: ""
author: " "
institute: "How can we leverage R to create Indices that are statistically-sound, reproducible and auditable?"
date: "`r Sys.Date()`"
output:
...
See JIAF Review here: 
https://kmp.hpc.tools/attachments/b0d076c7-2f8f-4fc1-94cf-591996d56eda/JIAG_Aggregation_Methods_final%20review_v3.pptx 
...
in addition, a series of plotting and dash-boarding functions are available
...
```{r eval=FALSE, message=FALSE, warning=FALSE, collapse=T, comment='#'}
library(COINr)
library(dplyr)
library(reactable)
library(magrittr)
...
ASEM <- COINr::assemble(
  IndData = readxl::read_excel(here::here("data-raw","StandardFormat.xlsx"), sheet = "IndData"),
  IndMeta = readxl::read_excel(here::here("data-raw","StandardFormat.xlsx"), sheet = "IndMeta"),
  AggMeta = readxl::read_excel(here::here("data-raw","StandardFormat.xlsx"), sheet = "AggMeta") 
  )
``` 
]
...
ASEM <- COINr::getStats(ASEM, dset = "Raw")
...
```{r ,eval=FALSE}
ASEM <- COINr::checkData(ASEM, dset = "Raw")
...
ASEM <- COINr::denominate(ASEM, dset = "Raw")

# get stats of denominated data
ASEM <- COINr::getStats(ASEM, dset = "Denominated")
...
![Risk](images/treat.png) 

```{r Winsorisation, fig.width=3, fig.height=3, echo=FALSE, dev='ragg_png', dpi=300}
# COINr::iplotIndDist2(ASEM, 
#               dsets = c("Denominated", "Treated"), 
#               icodes = "Services", 
#               ptype = "Scatter")
...
To check differences, check calculations for pre-aggregated data to make sure that they are correct. If we only have normalized and aggregated data, we can still at least check the aggregation stage as follows: Assuming that the indicator columns in pre-aggregated data are normalized, we can first manually create a normalized data set: 

```{r, message=FALSE}
Aggregated_Data <- ASEM$Data$Aggregated # extract aggregated data set (as a data frame)

# assemble new COIN only including pre-aggregated data
ASEM_preagg <- COINr::assemble(IndData = Aggregated_Data,
      IndMeta = readxl::read_excel(here::here("data-raw","StandardFormat.xlsx"), sheet = "IndMeta"),
      AggMeta = readxl::read_excel(here::here("data-raw","StandardFormat.xlsx"), sheet = "AggMeta") ,
      preagg = TRUE)
```
...
copying the pre-aggregated data, but removing any aggregation columns.
...
Next aggregating these columns using COINr.

```{r}
ASEM_preagg <- COINr::aggregate(ASEM_preagg, 
                         dset = "Normalised", 
                         agtype = "arith_mean")

# check data set names names(ASEM_preagg$Data)
```
]
...
## and then check whether  data frames are the same! 
...
Double-checking calculations is tedious but in the process is usefull. 

If the results are *not* the same, `all_equal()` will give some information about the differences.
]
...
Cronbach's alpha calculation should be done for any group of indicators (good if above 0.6 to 0.7). It measures reliability (i.e. how well a test measures what it should: measure of the stability of test scores), or internal consistency.
...
![Risk](images/frame.png) 

```{r fig.width=3, fig.height=3, echo=FALSE, dev='ragg_png', dpi=300}
#COINr::plotframework(ASEM)
...
# and compare
comp <- COINr::compTable(ASEM, ASEM_NoPolitical, 
                 dset = "Aggregated", isel = "Index",
...
## Uncertainty analysis:  test the overall effect of assumptions

.pull-left[
 
```{r, eval=FALSE}
# Noise to be applied to weights
nspecs <- data.frame(AgLevel = c(2,3), NoiseFactor = c(0.25,0.25))
# List specifying assumptions to vary and alternatives
SAspecs <- list(
...
UAresults <- COINr::sensitivity(ASEM, 
  v_targ = "Index", # The target variable to perform analysis on.
  SA_specs = SAspecs,
  N = 10,  # number of Monte Carlo replications.
  SA_type = "UA")# type of analysis to run
...
## Sensitivity analysis:  understanding the individual contributions of assumptions

.pull-left[
 
```{r, eval=FALSE}
# run uncertainty analysis
SAresults <- COINr::sensitivity(ASEM,
          v_targ = "Index", # The target variable to perform analysis on. 
          SA_specs = SAspecs,  # A list which specifiyng perturbation 
          N = 10,# number of Monte Carlo replications.
          SA_type = "SA",# type of analysis to run
          Nboot = 1000)# Number of bootstrap draws for estimates of confidence intervals

# plot shows the effect of the three uncertainties that we introduced: 
# the imputation type, the normalisation type, and the weights. 
...
## Present summary table: getResults
...
## Interactions between Sub Indices: iplotIndDist2 

```{r fig.width=3,  fig.asp = 0.8, echo=FALSE, dev='ragg_png', dpi=300}
# options(htmltools.dir.version = FALSE, 
#         htmltools.preserve.raw = FALSE,
#         tibble.width = 60)
# COINr::iplotIndDist2(ASEM, 
...
## Display the index on a Map: iplotMap
...
## COINr::resultsDash(ASEM, dset = "Aggregated")



![Risk](images/result.png) 

---

class: inverse, left, middle

# Test Case → Putting this in practice!

Applying this workflow to field example: what do we get?

 * Protection Severity Venezuela
 
 * Protection Severity Colombia
...
<i class="fa fa-check-square fa-fw fa-2x"></i> __TAKEAWAY #1__: Strictly Use "_curated & published_" data for your sub-indicators - Data should be available on Humanitarian Data eXchange (data.humdata.org)
 
<i class="fa fa-check-square fa-fw fa-2x"></i> __TAKEAWAY #2__: Use CoinR Indicator documentation structure in Excel (_IndData, IndMeta, AggMeta_) - This allows for better discoverability!
 
<i class="fa fa-check-square fa-fw fa-2x"></i> __TAKEAWAY #3__: Cover yourself for audit, i.e. Document your process with R Code & put it into Github - and/or Call for support / peer review! When using scripted analysis, this become very quick!!!
