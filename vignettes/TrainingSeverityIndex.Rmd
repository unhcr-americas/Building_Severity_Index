...
CHALLENGE: Summarizing and __condensing__ the information of a plurality of underlying indicators into a single measure, in a way that __accurately__ reflects the underlying concept
...
To bring in the humanitarian world, the same level of statistical quality than in development (aka making the "__nexus__"), as son as Humanitarian Severity is defined and indicators preselected, and before we can visualise the results, the following steps are required: 
...
## Using scripts rather than click
 ...
 * Visualisations of the index measurement quality

]


.pull-right[
__Scripting in R__ not only address the inherent limitation of Excel but also ensure:
 * Full documentation in a predictable format
 * Automatic index audit functions
...
.pull-left[
To build a composite indicator,  we need three data frames, __prepared and assembled in Excel__ according to a specific format with predefined column name
...
## Weighting indicators: balancing importance
...
The [Compind](https://rdrr.io/cran/Compind/) packages provided a series of method
