...
geo@data$UnitCode <- geo@data$ADM2_PCODE 
```
...
Cronbach’s alpha, (or coefficient alpha), developed by Lee Cronbach in 1951, measures reliability (i.e. how well a test measures what it should: measure of the stability of test scores), or internal consistency. If the coefficient is negative, there might be an issue with the set-up direction of the indicators.
...
print(COINr::plotCorr(SeverityIndex, dset = "Aggregated", 
                icodes = IndCompCode, 
                aglevs = IndCompLev, 
                pval = 0))
}
...
#                 aglev = 4, # The aggregation level to collect the indicator data from (this needs to be specified)
...
mapsf::mf_prop_typo( 
  x = geo2, 
  var = c("x_PopTot", "x_Severidad_expertos"),
  inches = .05, 
  border = "tomato4",
  alpha = .8,
  val_max = 90000, 
